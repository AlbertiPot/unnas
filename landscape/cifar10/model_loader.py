...
def load_unnas_evaled_nets(model_file=None):
    from pycls.models.nas.nas import NetworkCIFAR
    from pycls.models.nas.genotypes import GENOTYPES

    
    arch = 'v1cls_colc10seed9999'
    genotype = GENOTYPES[arch]
    model = NetworkCIFAR(C=36, num_classes=10, layers=20, auxiliary=True, genotype = genotype)
    print(arch, genotype)
    ...
    if model_file:
        ...
        weights = {}
        for k,v in stored['model_state'].items():
            if ("total_params" in k) or ("total_ops" in k):
                continue
            if 'net_.' in k:
                weights[k.replace('net_.', '')] = v
