...
class Network(nn.Module):
    ...
    def forward(self, input):
        ...
        logits = self.classifier(s1, input.shape[2:]) # cls_logits, col_logits
        return logits

    def _loss(self, input, target):
        logits = self(input)
        l_cls = self._criterion[0](logits[0],target[0])
        l_aux = self._criterion[1](logits[1],target[1])
        l = l_cls+l_aux/(l_aux/l_cls).detach()
        return l
...
class NAS_Search(nn.Module):
    ...
    def __init__(self):
        ...
        if cfg.TASK == 'seg':
            ...
            criterion = [nn.CrossEntropyLoss().cuda(), nn.CrossEntropyLoss().cuda()]
    ...
    def _loss(self, inputsB, labels):
        return self.net_._loss(inputsB, labels)
